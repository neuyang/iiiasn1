...
void H225ValueNotationTests()
{
	...
	const char addrs[] = "ipAddress : {\n"
						 ...
						 "  port 1051\n"
						 "}";
	TEST("TransportAddress (AVN)" , env ,addr ,addr2, addrs); 
	...
	TEST("SEQUENCE OF AliasAddress (AVN)" , env ,aliases, aliases2, as); 
...
void H225PERTests()
{
	...
	TEST("EndpointType (PER)", env ,ep, ep1, 
	"\x28\xC0\x26\x00\x53\x4C\x2F\x49"
	...
	TEST("RasMessage (PER)", env ,rasMessage,rm1, 	 
...
//	setup.set_presentationIndicator().select_presentationAllowed();
	...
	TEST("H323_UserInformation (PER)", env ,ui, ui2, 
	...
	TEST("RegistrationConfirm (PER)", env , rm3, rm4,
